...
/**
 * This application do console game for crosses zeros in one java class
 */
public class ConsoleGame {
    ...
    private static byte defineWinner(char sign, char[] box) {
        int[][] winConditions = {
    ...
    private static void logBattlefield(char[] box) {
    ...
    private static boolean clearBattlefield(boolean boxEmpty, char[] box) {
        if (!boxEmpty) {
            for (int i = 0; i < 9; i++) {
                box[i] = ' ';
            }
    ...
    private static void fillBattlefield(char[] box) {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            logBattlefield(box);
            boxEmpty = clearBattlefield(boxEmpty, box);


            if (winner != 0 || !boxEmpty) {
                announceGameResult(winner);
                break;  // Exit the loop if there is a winner or no more moves are available
            }
            ...
            winner = defineWinner('O', box) == 1 ? 2 : defineWinner('X', box);
        }
    }
}
